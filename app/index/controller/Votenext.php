...
class Votenext
{
  ...
  public function onLogin(Request $request)
  {
    ...
    // 如果 SOEID 重复了则返回 301
    $citi_vote_user_next = Db::table('citi_vote_user_next')->select();
    foreach ($citi_vote_user_next as $key => $item) {
    ...
    Db::table('citi_vote_user_next')->insert($vote_user);
  ...
  public function onVote(Request $request)
  {
    ...
    foreach ($choice_name as $key => $item) {
      ...
      Db::table('citi_vote_list_next')->insert($vote_list);
    }
  ...
  public function getVoteRes(Request $request)
  {
    ...
    foreach ($users_name as $key => $item) {
      $res = Db::table('citi_vote_list_next')->where('choice_name', $item)->select();
